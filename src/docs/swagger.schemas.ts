...
export const swaggerSchemas: Record<string, OpenAPIV3.SchemaObject> = {
  ...
  SearchProfilesRequest: {
    type: 'object',
    description:
      '**Send these fields in POST /search** after user selects filters. All fields optional.',
    properties: {
      name: { type: 'string', example: 'Ahmed', description: 'Partial match on profile name' },
      age_range: {
        type: 'string',
        example: '21-26',
        description: 'Age bucket value from GET /reference/search-filters → data.age_buckets[].value',
      },
      age_from: { type: 'integer', example: 22, description: 'Alternative to age_range' },
      age_to: { type: 'integer', example: 35 },
      height_from: { type: 'string', example: '5ft 0in', description: 'Min height (string like 5ft 4in or inches e.g. 64)' },
      height_to: { type: 'string', example: '6ft 2in' },
      marital_status: {
        oneOf: [{ type: 'string' }, { type: 'array', items: { type: 'string' } }],
        example: ['Never Married', 'divorced'],
        description: 'Use value from data.marital_status[].value',
      },
      sect: {
        oneOf: [{ type: 'string' }, { type: 'array', items: { type: 'string' } }],
        example: '1',
        description: 'Caste/sect ID from GET /castes',
      },
      cast: {
        oneOf: [{ type: 'string' }, { type: 'array', items: { type: 'string' } }],
        example: '5',
        description: 'Sub-caste ID from GET /castes/{id}/sub-castes',
      },
      country: {
        oneOf: [{ type: 'string' }, { type: 'array', items: { type: 'string' } }],
        example: '101',
        description: 'Country ID from GET /locations/countries',
      },
      ...
      highest_education: {
        oneOf: [{ type: 'string' }, { type: 'array', items: { type: 'string' } }],
        example: 'Graduate',
        description: 'From data.highest_education',
      },
      employed_in: {
        oneOf: [{ type: 'string' }, { type: 'array', items: { type: 'string' } }],
        example: 'private',
        description: 'From data.employed_in[].value',
      },
      mother_tounge: {
        oneOf: [{ type: 'string' }, { type: 'array', items: { type: 'string' } }],
        example: ['Hindi/Urdu', 'English'],
        description: 'From data.mother_tounge',
      },
      income_min_lakh: { type: 'number', example: 0, description: 'Income slider min (lakhs)' },
      income_max_lakh: { type: 'number', example: 15, description: 'Income slider max (lakhs)' },
      annual_income: {
        oneOf: [{ type: 'string' }, { type: 'array', items: { type: 'string' } }],
        example: 'Rs. 10 - 12 Lakh',
        description: 'Exact bracket from data.annual_income[].value',
      },
      page: { type: 'integer', example: 1, default: 1 },
      limit: { type: 'integer', example: 20, default: 20, description: 'Max 50' },
    },
  },
  SearchAgeBucket: {
    type: 'object',
    properties: {
      value: { type: 'string', example: '21-26', description: 'Send as age_range in POST /search' },
      label: { type: 'string', example: '21 - 26' },
      age_from: { type: 'integer', example: 21 },
      age_to: { type: 'integer', example: 26 },
    },
  },
  SearchLabelValue: {
    type: 'object',
    properties: {
      value: { type: 'string', example: 'divorced' },
      label: { type: 'string', example: 'Divorced' },
    },
  },
  SearchIncomeBracket: {
    type: 'object',
    properties: {
      value: { type: 'string', example: 'Rs. 10 - 12 Lakh', description: 'Send as annual_income or use min_lakh/max_lakh slider' },
      label: { type: 'string', example: 'Rs. 10 - 12 Lakh' },
      min_lakh: { type: 'number', example: 10 },
      max_lakh: { type: 'number', example: 12 },
    },
  },
  SearchFiltersData: {
    type: 'object',
    description: 'Filter options for the mobile Filters screen — nothing is sent TO this endpoint',
    properties: {
      age_buckets: {
        type: 'array',
        items: { $ref: '#/components/schemas/SearchAgeBucket' },
      },
      marital_status: {
        type: 'array',
        items: { $ref: '#/components/schemas/SearchLabelValue' },
      },
      mother_tounge: {
        type: 'array',
        items: { type: 'string' },
        example: ['Hindi/Urdu', 'English', 'Bengali'],
      },
      highest_education: {
        type: 'array',
        items: { type: 'string' },
        example: ['Graduate', 'Post Graduate', 'Medical - MBBS'],
      },
      employed_in: {
        type: 'array',
        items: { $ref: '#/components/schemas/SearchLabelValue' },
      ...
      annual_income: {
        type: 'array',
        items: { $ref: '#/components/schemas/SearchIncomeBracket' },
      },
      height: {
        type: 'object',
        properties: {
          description: { type: 'string', example: 'Send height_from and height_to as strings like "5ft 0in" or inch numbers' },
          example_from: { type: 'string', example: '5ft 0in' },
          example_to: { type: 'string', example: '6ft 2in' },
        },
      },
      sect: {
        type: 'object',
        properties: {
          description: { type: 'string', example: 'Use GET /castes — send sect ID in POST /search' },
        },
      },
      cast: {
        type: 'object',
        properties: {
          description: { type: 'string', example: 'Use GET /castes/{id}/sub-castes — send cast ID in POST /search' },
        },
      },
      country: {
        type: 'object',
        properties: {
          description: { type: 'string', example: 'Use GET /locations/countries — send country ID in POST /search' },
        },
      },
    },
  },
  SearchFiltersResponse: {
    ...
    properties: {
      success: { type: 'boolean', example: true },
      message: { type: 'string', example: 'Search filter options for mobile app' },
      data: { $ref: '#/components/schemas/SearchFiltersData' },
    },
  },
  SearchProfileItem: {
    type: 'object',
    description: 'Matching member profile with resolved *_name fields',
    properties: {
      ...
      city_name: { type: 'string', example: 'Mumbai' },
      marital_status: { type: 'string', example: 'Never Married' },
      mother_tounge: { type: 'string', example: 'Hindi/Urdu' },
      sect: { type: 'string', example: '1' },
      sect_name: { type: 'string', example: 'Sunni' },
      cast: { type: 'string', example: '5' },
      cast_name: { type: 'string', example: 'Hanafi' },
      highest_education: { type: 'string', example: 'Graduate' },
      occupation: { type: 'string', example: 'Software Engineer' },
  ...
  SearchProfilesResponse: {
    ...
    properties: {
      ...
      data: {
        type: 'array',
        items: { $ref: '#/components/schemas/SearchProfileItem' },
      },
      meta: {
        type: 'object',
        properties: {
  ...
  PartnerPreferencesRequest: {
    type: 'object',
    description:
      'All fields optional — partial save supported. Send only fields the user filled. Use null or "" to clear a value.',
    properties: {
      marital_status: { type: 'string', example: 'Never Married', nullable: true },
      age_from: { type: 'integer', example: 22, nullable: true },
      age_to: { type: 'integer', example: 32, nullable: true },
      highest_education: { type: 'string', example: 'Graduate', nullable: true },
      mother_tounge: { type: 'string', example: 'Hindi/Urdu', nullable: true },
      sect: { type: 'string', example: '1', description: 'Caste/sect ID (string or number)', nullable: true },
      cast: { type: 'string', example: '5', description: 'Sub-caste ID (string or number)', nullable: true },
      height_from: { type: 'string', example: '5ft 0in', nullable: true },
      height_to: { type: 'string', example: '6ft 0in', nullable: true },
      occupation: { type: 'string', example: 'Engineer', nullable: true },
      country: { type: 'string', example: '101', description: 'Country ID (string or number)', nullable: true },
      state: { type: 'string', example: '4023', nullable: true },
      city: { type: 'string', example: '57606', nullable: true },
      annual_income: { type: 'string', example: 'Rs. 10 - 12 Lakh', nullable: true },
    },
  },
  PartnerPreferencesGetResponse: {
    type: 'object',
    properties: {
      success: { type: 'boolean', example: true },
      message: { type: 'string', example: 'Partner preferences fetched' },
      data: {
        type: 'object',
        properties: {
          preferences: {
            type: 'object',
            nullable: true,
            description: 'Saved preferences (null if never saved)',
            properties: {
              marital_status: { type: 'string', nullable: true },
              age_from: { type: 'integer', nullable: true },
              age_to: { type: 'integer', nullable: true },
              highest_education: { type: 'string', nullable: true },
              mother_tounge: { type: 'string', nullable: true },
              sect: { type: 'string', nullable: true },
              sect_name: { type: 'string', example: 'Sunni' },
              cast: { type: 'string', nullable: true },
              cast_name: { type: 'string', example: 'Hanafi' },
              height_from: { type: 'string', nullable: true, example: '5ft 0in' },
              height_to: { type: 'string', nullable: true, example: '6ft 0in' },
          ...
          field_options: {
            type: 'object',
            description: 'Dropdown/range options for the partner preference form',
            properties: {
              fields: { type: 'array', items: { type: 'object' } },
              marital_status: { type: 'array', items: { type: 'object' } },
              age_buckets: { type: 'array', items: { type: 'object' } },
              highest_education: { type: 'array', items: { type: 'string' } },
              mother_tounge: { type: 'array', items: { type: 'string' } },
              occupation: { type: 'array', items: { type: 'string' } },
              annual_income: { type: 'array', items: { type: 'object' } },
            },
          },
        },
      },
